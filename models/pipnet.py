...
#from uxnet_features import encoder_uxnet_features
...
class PIPNet(nn.Module):
    ...
    def forward(self, xs, inference=False):
        ...
        pooled = self._pool(proto_features)     # (bs,ps,1,1,1) -> (bs,ps):
                                                #   prototype presence in xs
        if inference:
            # during inference, ignore all prototypes that have 0.1 similarity 
            # or lower
...
base_architecture_to_features = {
    '3Dresnet18': video_resnet18_features,
    '3Duxnet': encoder_uxnet_features
    }
"""
...
def get_network(
    ...
    if features_name.startswith('VIDEO') or features_name.startswith('UXNET'):
